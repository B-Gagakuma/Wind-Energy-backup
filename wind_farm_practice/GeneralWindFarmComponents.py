...
class MeanAEP(Component):
    """ Gives the mean AEP of 1 direction run multiple times """

    def __init__(self, nSamples):

        super(MeanAEP, self).__init__()
        ...
        for i in range(nSamples):
            ...
            self.add_param('AEP%s'%i, 0.0, desc='Power output from each sampled wind direction')

        # define output
        self.add_output('MeanAEP', 0.0, desc='Power output from each sampled wind direction')
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        for i in range(nSamples):
            sum_AEP += params['AEP%s'%i]

        unknowns['MeanAEP'] = -1. * sum_AEP/float(nSamples)
    ...
    def linearize(self, params, unknowns, resids):
        ...
        J = {}
        for i in range(nSamples):
            J['MeanAEP','AEP%s'%i] = -1./float(nSamples)
