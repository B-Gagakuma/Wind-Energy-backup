...
def call_floris(turbineX, turbineY, turbineZ, rotorDiameter, ratedPower, windDirections, windSpeeds, windFrequencies, shearExp):
    ...
    return nTurbines, nDirections, turbineXw, turbineYw, Vinf

def power_call(dirs):

    nTurbines, nDirections, turbineXw, turbineYw, Vinf = call_floris(turbineX, turbineY, turbineZ, rotorDiameter, ratedPower,
                                                               dirs, windSpeeds, windFrequencies, shearExp)
...
def optVariance(turbineX, turbineY, turbineZ, rotorDiameter, ratedPower, windDirections, windSpeeds, windFrequencies, shearExp):
    ...
    for i in range(nDirections):
        meanPower += windFrequencies[i] * dir_powers[i]

    for i in range(nDirections):
        varPower += windFrequencies[i] * np.abs((dir_powers[i] - meanPower))**2

    # Trapezoidal rule
    # I1 = np.trapz(windFrequencies*dir_powers)
    # I3 = np.trapz(windFrequencies*dir_powers**2) - I1**2


    # Simpson's rule for E[p] calculation
    # I2 = integrate.simps(windFrequencies*dir_powers)
    # I4 = integrate.simps(windFrequencies*dir_powers**2) - I2**2

    # print 'rect_', meanPower
    # print 'trapz', I3
    # print 'Simps', I4
    ...
    #
    # freq_func = scipy.interpolate.CubicSpline(Directions, Frequencies)
    #
    # # Legendre-Gauss quadrature
    # order = 10
    # [lg_mean, err] = LegendreGaussQuadrature(power_call,freq_func, order, 0., 2.*np.pi)
    # print 'lg_mean', lg_mean

    return meanPower, varPower
